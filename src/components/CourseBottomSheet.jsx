          ...
          >
            ...
            >
              ...
              <div className="px-6 sm:px-8 pt-5 pb-4 bg-slate-50 border-b border-slate-200 flex items-center justify-between shrink-0">
                <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-[6px] bg-[#0D62FE]/10 border border-[#0D62FE]/20 text-[#0D62FE] text-[11px] font-mono font-bold tracking-wide">
                ...
                <button
                  onClick={onClose}
                  className="w-8 h-8 rounded-[8px] bg-white hover:bg-slate-200 text-[#1C2533] flex items-center justify-center transition-colors cursor-pointer border border-slate-200 shadow-2xs"
                  aria-label="Close"
                >
                  <X className="w-4 h-4" />
                </button>
              ...
              >
                ...
                {(course.toolGroups || course.toolsCovered) && (course.toolGroups || course.toolsCovered).length > 0 ? (
                  <div className="space-y-3 pt-1">
                    <h4 className="text-[11px] font-bold text-[#1C2533] uppercase tracking-wider flex items-center gap-1.5 font-mono">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 items-start">
                      {(course.toolGroups || course.toolsCovered).map((group, idx, arr) => (
                        <div
                          key={idx}
                          className={`min-w-0 bg-slate-50 p-3.5 rounded-[12px] border border-slate-200 space-y-2 ${arr.length % 2 === 1 && idx === arr.length - 1 ? 'sm:col-span-2' : ''
                            }`}
                        >
                          ...
                          <div className="flex flex-wrap gap-1.5">
                            {group.tools.map((tool, tIdx) => (
                              <span
                                key={tIdx}
                                className="inline-flex items-center max-w-full px-2.5 py-1 rounded-[6px] bg-white border border-slate-200 text-[11px] font-semibold text-[#1C2533] shadow-2xs leading-normal text-left break-words"
                              >
                                {tool}
                              </span>
                            ))}
                  ...
                  <div className="space-y-2.5 pt-1">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {course.popularAreas.map((area, idx) => (
                        <span
                          key={idx}
                          className="px-3 py-1.5 rounded-[8px] bg-slate-50 border border-slate-200 text-xs font-bold text-[#1C2533] shadow-2xs flex items-center gap-1.5"
                        >
                          <span className="w-1.5 h-1.5 rounded-full bg-[#0D62FE]" />
                          {area}
                        </span>
                      ))}
                    </div>
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        {course.workflow.map((step, idx) => (
                          <React.Fragment key={idx}>
                            <div className="px-3 py-1.5 bg-white rounded-[8px] border border-slate-200 text-xs font-bold text-[#1C2533] shadow-2xs flex items-center gap-1.5">
                              <span className="w-4 h-4 rounded-full bg-[#0D62FE] text-white text-[9px] font-black flex items-center justify-center">
                                {idx + 1}
                              </span>
                              <span>{step}</span>
                            </div>
                            {idx < course.workflow.length - 1 && (
                              <ArrowRight className="w-3.5 h-3.5 text-[#0D62FE] shrink-0" />
                            )}
                          </React.Fragment>
                        ))}
                      </div>
                ...
                {((course.keyModules && course.keyModules.length > 0) || (course.outcomes && course.outcomes.length > 0)) && (
                  <div className="space-y-2.5 pt-3 border-t border-slate-200">
                    <h4 className="text-[11px] font-bold text-[#1C2533] uppercase tracking-wider flex items-center gap-1.5 font-mono">
                      <GraduationCap className="w-3.5 h-3.5 text-[#0D62FE]" />
                ...
                {course.careerPaths && course.careerPaths.length > 0 && (
                  <div className="space-y-2.5 pt-3 border-t border-slate-200">
                    <h4 className="text-[11px] font-bold text-[#1C2533] uppercase tracking-wider flex items-center gap-1.5 font-mono">
                      <Briefcase className="w-3.5 h-3.5 text-[#0D62FE]" />
                ...
                {/* Local SEO Badge */}
                {course.seoLocation && (
                  <div className="p-3 rounded-[10px] bg-slate-50 border border-slate-200 text-[#687282] text-xs font-medium flex items-center gap-2">
                    <MapPin className="w-4 h-4 text-[#0D62FE] shrink-0" />
              ...
              <div className="p-4 sm:p-5 bg-slate-50 border-t border-slate-200 flex items-center justify-between gap-4 shrink-0">
                <div>
                  <p className="text-xs font-bold text-[#1C2533] flex items-center gap-1.5">
                    <ShieldCheck className="w-4 h-4 text-[#0D62FE]" />
                ...
                <Button
                  ...
                  variant="primary"
                  size="md"
                  className="bg-[#0D62FE] hover:bg-[#0052FF] text-white shadow-md shadow-blue-500/25"
                >
                  Enquire Now
                </Button>
              </div>
